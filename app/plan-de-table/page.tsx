      ...
      <div className="w-full max-w-[450px] bg-white rounded-[2.5rem] p-6 shadow-xl border border-slate-100/50 flex flex-col relative overflow-hidden min-h-[550px]">
        ...
        <div className="text-center mt-4 mb-6">
          <div className="inline-flex items-center gap-1 bg-amber-50 text-amber-600 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider mb-2">
            <Utensils className="w-3 h-3" /> Placement
          </div>
          ...
          <p className="text-xs text-slate-400 mt-1 px-4">
            Saisissez votre prénom ou votre nom pour découvrir votre placement dans la salle.
          </p>
        </div>
        ...
        <div className="relative mb-6">
          <input
            ...
            className="w-full pl-12 pr-4 py-4 bg-slate-50 focus:bg-white rounded-2xl text-sm font-bold border border-slate-100 focus:border-rose-200 outline-none transition-all placeholder:text-slate-400 text-slate-800 shadow-inner"
          />
          ...
          <AnimatePresence mode="wait">
            {searchQuery.trim().length < 2 ? (
              <motion.div
                ...
                className="text-center text-slate-300 py-8 space-y-2 my-auto"
              >
                <HelpCircle className="w-12 h-12 mx-auto stroke-[1.5]" />
                <p className="text-xs font-bold uppercase tracking-wider">En attente de saisie</p>
                <p className="text-[11px] text-slate-400 max-w-[200px] mx-auto">Tapez au moins 2 lettres de votre prénom ou de votre nom.</p>
              </motion.div>
            ) : hasSearched && searchResults.length === 0 && !loading ? (
              <motion.div
                key="no-result"
                initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
                className="text-center bg-rose-50/50 border border-rose-100 p-6 rounded-3xl my-auto"
              >
                <p className="text-sm font-black text-rose-500">Nom introuvable</p>
                <p className="text-xs text-slate-500 mt-1">
                  Vérifiez l'orthographe. Si votre nom n'apparaît toujours pas, demandez de l'aide aux témoins de la soirée !
                </p>
              </motion.div>
            ) : (
              <motion.div
                  ...
                  >
                    ...
                    <div className="flex justify-between items-start">
                      <div className="space-y-1">
                        ...
                        <h3 className="text-base font-black tracking-tight text-white">{invite.name}</h3>
                      </div>
                    </div>
                    ...
                    <div className="mt-5 pt-4 border-t border-white/10 flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-lg font-black text-amber-300 tracking-tight mt-0.5">
                          {invite.table_name}
                        </p>
                      </div>
