...
import { 
  ...
  Church, Cross, Compass, Check, Landmark
} from 'lucide-react';
...
function RSVPContent() {
  ...
  const [attendanceMode, setAttendanceMode] = useState<'all' | 'custom'>('all');
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (guestId) {
          const { data: gData } = await supabase.from('invite').select('name').eq('id', guestId).single();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const finalCount = form.status === 'décliné' ? 0 : 1;
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        notes: form.notes,
        attending_civil: form.status === 'confirmé' ? form.attending_civil : false,
        attending_church: form.status === 'confirmé' ? form.attending_church : false,
        attending_reception: form.status === 'confirmé' ? form.attending_reception : false
      })
      .eq('id', guestId);
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="flex gap-4">
                <button type="button" onClick={() => setForm({...form, status: 'confirmé'})}
                  className={`flex-1 py-5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all border-2 ${form.status === 'confirmé' ? 'bg-slate-900 text-white border-transparent shadow-xl scale-105' : 'bg-white text-slate-400 border-slate-100'}`}>
                  Je serai là
                </button>
                  ...
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-6 overflow-hidden">
                    ...
                    <div className="p-6 bg-slate-50/50 rounded-3xl border border-slate-100 space-y-5">
                      ...
                      <div className="bg-slate-100/80 p-1 rounded-2xl flex gap-1">
                        <button
                          ...
                          className={`flex-1 py-3 rounded-xl font-black text-xs transition-all ${
                            attendanceMode === 'all' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'
                          }`}
                        >
                          Tous les moments
                        </button>
                        <button
                          ...
                          className={`flex-1 py-3 rounded-xl font-black text-xs transition-all ${
                            attendanceMode === 'custom' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'
                          }`}
                        >
                          Sur-mesure
                        </button>
                      ...
                      <div className="space-y-3">
                        ...
                        <button
                          type="button"
                          disabled={attendanceMode === 'all'}
                          onClick={() => setForm({ ...form, attending_civil: !form.attending_civil })}
                          className={`w-full p-4 rounded-2xl border-2 flex items-center justify-between transition-all ${
                            form.attending_civil
                              ? 'border-rose-400 bg-rose-50/30 text-rose-600'
                              : 'border-slate-100 bg-white text-slate-400'
                          }`}
                        >
                          <div className="flex items-center gap-3 font-black text-sm">
                            <Landmark size={18} /> Mairie
                          </div>
                          <div className={`w-6 h-6 rounded-full flex items-center justify-center ${form.attending_civil ? 'bg-rose-500 text-white' : 'bg-slate-100 text-slate-300'}`}>
                        ...
                        <button
                          type="button"
                          disabled={attendanceMode === 'all'}
                          onClick={() => setForm({ ...form, attending_church: !form.attending_church })}
                          className={`w-full p-4 rounded-2xl border-2 flex items-center justify-between transition-all ${
                        ...
                        >
                          <div className="flex items-center gap-3 font-black text-sm">
                            <Cross size={18} /> Église
                          </div>
                          <div className={`w-6 h-6 rounded-full flex items-center justify-center ${form.attending_church ? 'bg-rose-500 text-white' : 'bg-slate-100 text-slate-300'}`}>
                            <Check size={14} strokeWidth={3} />
                          </div>
                        </button>
                        ...
                        <button
                          type="button"
                          disabled={attendanceMode === 'all'}
                          onClick={() => setForm({ ...form, attending_reception: !form.attending_reception })}
                          className={`w-full p-4 rounded-2xl border-2 flex items-center justify-between transition-all ${
                        ...
                        >
                          <div className="flex items-center gap-3 font-black text-sm">
                            <GlassWater size={18} /> Réception & Dîner
                          </div>
                          <div className={`w-6 h-6 rounded-full flex items-center justify-center ${form.attending_reception ? 'bg-rose-500 text-white' : 'bg-slate-100 text-slate-300'}`}>
                            <Check size={14} strokeWidth={3} />
                          </div>
                        </button>
                      </div>
                    ...
                    <textarea 
                      ...
                      className="w-full p-6 bg-slate-100/50 rounded-3xl border-2 border-slate-200 font-bold outline-none text-slate-800 text-sm h-32 resize-none shadow-inner placeholder:text-slate-400 placeholder:font-bold focus:border-slate-300 transition-all" 
                      value={form.notes} 
              ...
              <button type="submit" disabled={sending || !guestId}
